...
function mapSessionResponse(data: Record<string, unknown>): ReadingSession {
  ...
  return {
    ...
    tokens:           (data.tokens as any[] ?? []).map(t => ({
      text:   String(t.text),
      type:   t.type,
      status: t.status,
...
export function logWordLookup(_userId: string): void {}
export function logDwellTime(_userId: string, _sessionId: string, _elapsedMs: number): void {}
